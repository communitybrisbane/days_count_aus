...
- プロフィール作成後、選択したメインモードの公式グループ + 公式Q&Aグループ「WH Ask Anything」（`official-ask-anything`）に自動参加。
...
- **全ユーザーの公開（public）かつアクティブ（active）な投稿** をスコアベースランキングで表示。ブロックは**完全相互非表示**：自分がブロックした相手の投稿に加え、自分をブロックした相手（`blockedBy`）の投稿もフィードから除外（2026-07-04〜）。
...
- **テキスト入力**: **画像（またはグラデーション）の中央に直接入力**。幅は画像の70%・最大10行・350文字以内（約35文字/行×10行）（ASCII + 絵文字のみ）。入力欄は文章量に応じて自動で伸びる（内側スクロールなし）。
