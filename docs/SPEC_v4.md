...
- **初期参加**: 自分のメインモードのグループに自動参加。メインモード変更時は自動で入れ替え（旧を退出→新に参加）。
- **任意参加**: 他モードのモードグループにも検索（Find）から参加可能。レベル・スロット制限の対象外（無料枠）。自分のメインモード以外のモードグループはチャット画面から退出可能（メインモードのグループは退出不可、「Official」バッジ表示）。
- 公式グループはメンバー上限なし。
