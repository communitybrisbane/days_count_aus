...
- 各フォーカスモードごとに1つのモードグループが存在（`isOfficial: true`）。ユーザーのメインモード変更時に自動参加/退出。カードに「by mode」表示。
- 公式グループはメンバー上限なし。
- グループは「公式（モード）グループ + ユーザー作成コミュニティ」の2種類のみ（旧・趣味グループ Hobby Groups は 2026-07-02 に全削除）。検索画面に表示されるのはユーザー作成コミュニティのみ。
